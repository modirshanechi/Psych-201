...
for experiment in range(1,5):
    ...
    for participant in subjects:
        ...
        rts = df_participant[7].astype(float)*1000 #from sec to ms
        ...
        prompt = prompt[:-1]
        print(prompt)
        all_prompts.append({'text': prompt, 'experiment': 'chambon2020feedback/'+str(experiment), 'participant': str(participant),'RTs':json.dumps(list(rts))})
